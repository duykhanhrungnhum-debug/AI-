...
class FakeResearcher:
    def fetch(self, uri):
        return ResearchDocument(
            uri=uri, content="evidence-backed learning material", content_hash="hash-1",
            retrieved_at="2026-01-01T00:00:00+00:00", content_type="text/plain")


class ConflictResearcher:
    def fetch(self, uri):
        content = ("autonomous planning is a useful capability"
                   if uri.endswith("/support") else "autonomous planning is not a useful capability")
        return ResearchDocument(uri=uri, content=content, content_hash=uri.rsplit("/", 1)[-1],
                                retrieved_at="2026-01-01T00:00:00+00:00", content_type="text/plain")
...
def test_coordinator_connects_plan_research_and_verify():
    ...
    session = coordinator.run_once("learn autonomous planning", "https://example.test/planning",
...
def test_coordinator_does_not_loop_when_no_gap_exists():
    coordinator = AutonomousLearningCoordinator(researcher=FakeResearcher())
    existing = coordinator.learner.observe("autonomous planning is a verified capability",
...
def test_coordinator_discovers_and_researches_multiple_sources_without_auto_verifying():
    provider = StaticSearchProvider([SearchResult("https://example.test/a", title="A"),
                                     SearchResult("https://example.test/a", title="A duplicate"),
                                     SearchResult("https://example.test/b", title="B")])
    coordinator = AutonomousLearningCoordinator(research_planner=ResearchPlanner(provider, max_sources=2),
    ...
    assert [attempt.source_uri for attempt in session.attempts] == ["https://example.test/a", "https://example.test/b"]
...
def test_conflicting_sources_are_not_auto_verified():
    provider = StaticSearchProvider([SearchResult("https://example.test/support", title="support"),
...
def test_long_running_loop_has_explicit_cycle_bound_and_survives_cycle_failure():
    ...
    calls = {"count": 0}
    original = coordinator.run_once

    def run_once(goal, uri=None, evidence=None):
        calls["count"] += 1
        if calls["count"] == 2:
            raise RuntimeError("transient failure")
        return original(goal, "https://example.test/item", ["verified evidence"])
