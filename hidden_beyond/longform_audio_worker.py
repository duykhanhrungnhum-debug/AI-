...
def main():
    ...
    heartbeat("tts","Synthesizing Vietnamese voice")
    ...
    def synth(text:str,path:Path):
        if voice_mode=="zerotts-hamy":
            a=zerotts.synthesize(text,voice="hamy",cfg_scale=1.0,audio_temperature=0.78,audio_topk=25,audio_topp=0.95,audio_repetition_penalty=1.2)
            zerotts.save_audio(a,str(path))
        else:
            with wave.open(str(path),"wb") as w: piper_voice.synthesize_wav(text,w)
    ...
    for i,s in enumerate(segments,1):
        ...
        if i%20==0: heartbeat("tts",f"Synthesized {i}/{len(segments)} segments")
